    ...
    for i in range(len(line_list)):
        if len(token_list[i]) == 0:
            ...
            if i < 4  or i > len(line_list)- 5:
                ...
                prev_prev_label = 'same:2' if (token_list[i] == token_list[prev_prev_subscript]) else "diff"
                prev_label = 'same:2' if (token_list[i] == token_list[previous_subscript]) else "diff"
                next_label = 'same:2' if (token_list[i] == token_list[next_subscript]) else "diff"
                next_next_subscript = 'same:2' if (token_list[i] == token_list[next_next_subscript]) else "diff"
