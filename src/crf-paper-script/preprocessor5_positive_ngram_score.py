...
def Standardize(path, dest_dir,  sep):
    ...
    for line in file_obj:
        if len(line)<13:
            ...
            ng3 = 'ng3:'+ str( -1 * string.atof(word_list[6]))
            ng4 = 'ng4:'+ str( -1 * string.atof(word_list[7]))
            ng3pr = 'ng3pr:'+ str( -1 * string.atof(word_list[8]))
...
def GetSsrFeature(path, dest_dir,  sep):
    ...
    for line in file_obj:
        if len(line)<3:
            ...
            join = '_'.join(word_list)
            newLine = line[:54] + join
            word_list = newLine.split()

        token = word_list[0].strip()
        pos = word_list[1].strip()
        word = word_list[2].strip()
        sem = word_list[3].strip()
        label = word_list[4]
