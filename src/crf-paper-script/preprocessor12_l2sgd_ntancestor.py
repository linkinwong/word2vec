...
################################################################
    script_file = code_dir + 'chunker12_ntancestor_sgd.py'
    path = source_path
...
def Standardize(path, dest_dir,  sep):
    ...
    for k in range(len(line_list)):
        line = line_list[k]
        if len(line)<13 or len(line.split())<3:
            label = ''
        else:
            word_list = line.split()
        ...
        if len(line)<13:
            ...
            line_format = (
                ...
                %(label, sep, token,sep,pos, sep,word,sep,sem, sep, patial, sep,
                  pp, sep, p, sep, n,sep, nn, sep, nt_ancestor))
